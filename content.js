...
document.addEventListener('click', (e) => {
  ...
  if (link) {
    ...
    if (href) {
      ...
      const newName = prompt('新しい科目名を入力してください（空にすると元の名前にリセットされます）:\n\n現在の表示: ' + currentName, currentName);
      
      if (newName !== null) { // キャンセルしていない場合
        chrome.storage.local.get(['subjectTitles'], (result) => {
          const titles = result.subjectTitles || {};
          
          if (newName.trim() === '') {
            delete titles[key];
            alert('カスタム名称を削除しました。ページをリロードすると元の名前に戻ります。');
          } else {
    ...
    if (confirm('科目名の選択がキャンセルされました。編集モードを終了しますか？')) {
      selectionMode = false;
      document.body.style.cursor = '';
    } else {
      e.preventDefault();
...
}, true); // captureフェーズで実行して元のリンク動作をブロック
